...
export const GET_USER_LIKED_HISTORY_REQUEST = 'GET_USER_LIKED_HISTORY_REQUEST' as const
...
export const getUserLikedHistory = ():TypeUserThunkAction => async(dispatch, getState) => {
    ...
    const currentHistory = await database().ref(ref).once('value').then(snapshot => snapshot.val());
